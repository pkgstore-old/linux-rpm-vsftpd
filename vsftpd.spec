...
%{__mkdir_p} %{buildroot}%{_sysconfdir}/{vsftpd,pam.d,logrotate.d}
%{__mkdir_p} %{buildroot}%{_mandir}/man{5,8}
%{__mkdir_p} %{buildroot}%{_unitdir}
%{__mkdir_p} %{buildroot}%{_generatorsdir}
%{__install} -m 755 vsftpd  %{buildroot}%{_sbindir}/vsftpd
%{__install} -m 600 vsftpd.conf %{buildroot}%{_sysconfdir}/vsftpd/vsftpd.conf
%{__install} -m 644 vsftpd.conf.5 %{buildroot}/%{_mandir}/man5/
%{__install} -m 644 vsftpd.8 %{buildroot}/%{_mandir}/man8/
%{__install} -m 644 RedHat/vsftpd.log %{buildroot}%{_sysconfdir}/logrotate.d/vsftpd
%{__install} -m 644 %{SOURCE2} %{buildroot}%{_sysconfdir}/pam.d/vsftpd
%{__install} -m 600 %{SOURCE3} %{buildroot}%{_sysconfdir}/vsftpd/ftpusers
%{__install} -m 600 %{SOURCE4} %{buildroot}%{_sysconfdir}/vsftpd/user_list
%{__install} -m 744 %{SOURCE6} %{buildroot}%{_sysconfdir}/vsftpd/vsftpd_conf_migrate.sh
%{__install} -m 644 %{SOURCE7} %{buildroot}%{_unitdir}
%{__install} -m 644 %{SOURCE8} %{buildroot}%{_unitdir}
%{__install} -m 644 %{SOURCE9} %{buildroot}%{_unitdir}
%{__install} -m 755 %{SOURCE10} %{buildroot}%{_generatorsdir}

%{__mkdir_p} %{buildroot}/%{_var}/ftp/pub
%{__mkdir_p} %{buildroot}%{_libexecdir}

# Install vsftpd-ssl-pass-dialog.
%{__install} -m 755 %{SOURCE920} \
  %{buildroot}%{_libexecdir}/vsftpd-ssl-pass-dialog

# Install vsftpd-ssl-gencerts.
%{__install} -m 755 %{SOURCE921} \
  %{buildroot}%{_libexecdir}/vsftpd-ssl-gencerts
...
- Merge review:
- fix using %%{_var}, %%{_sbindir} and %%{_sysconfigdir} macros for files and install
- fix BuildRoot
